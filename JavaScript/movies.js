...
const db = createClient(
    "https://vkfeqqvkuqwdpsonjhwa.supabase.co",
...
function renderPelicula(p) {
    ...
    <div class="detail-hero">
        <div class="detail-poster-wrap">
            <img class="detail-poster" src="${p.poster}" alt="${p.title}">
            ${adultBadge}
        </div>
        <div class="detail-info">
            <h1 class="detail-title">${p.title} <span class="detail-year">(${year})</span></h1>
            ${p.original_title !== p.title
            ? `<p class="detail-original">${p.original_title}</p>`
            : ""}
